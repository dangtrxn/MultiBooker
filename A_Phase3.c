...
void *book(void* tid){
    ...
    //Assume there is a buy now option and a holding option
    //This is where a deadlock will occur
    if(customer_id % 2 == 0){
        //Half of threads will hold the tickets and possibly buy later
        pthread_mutex_lock(&ticket_hold_mutex);
        usleep(1000);
        pthread_mutex_lock(&payment_mutex);
    }
    else{
        //Half of threads will immediately buy tickets
        pthread_mutex_lock(&payment_mutex);
        pthread_mutex_lock(&ticket_hold_mutex);
    }
